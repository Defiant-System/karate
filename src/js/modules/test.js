...
let Test = {
	init(APP) {
		...
		setTimeout(() => {
			...
			APP.editor.els.tree.find(`> .item:nth(2) > .children > .item:nth(7) > span`).trigger("click");
		}, 800);
		...
		setTimeout(() => APP.game.dispatch({ type: "window.keydown", char: "a" }), 300);
		setTimeout(() => APP.game.dispatch({ type: "window.keyup", char: "a" }), 350);
